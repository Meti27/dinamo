...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="finale-") as tmp:
        ...
        def still(i):
            """the reduced-motion / no-AVIF fallback for this beat

            Written here rather than by hand: it is the closed box these frames
            end on, so it has to be regenerated whenever they are. It was not,
            once, and the page kept serving the previous clip's box.
            """
            flat = np.clip(frames[i].astype(np.float32) - correction, 0, 255)
            buf = flat.astype(np.uint8).tobytes()
            png = os.path.join(ROOT, "public/finale-still.png")
            run(["ffmpeg", "-v", "error", "-y", "-f", "rawvideo", "-pix_fmt", "rgb24",
                 "-s", f"{w}x{h}", "-i", "pipe:0",
                 "-vf", f"scale={DESKTOP_W}:{DESKTOP_W}:flags=lanczos", "-update", "1", png],
                input=buf)
            avif_out = os.path.join(ROOT, "public/finale-still.avif")
            run(["avifenc", "-y", "420", "-q", "60", "-s", "6", "-j", "all", png, avif_out],
                stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            run(["ffmpeg", "-v", "error", "-y", "-i", png, "-c:v", "libwebp",
                 "-quality", "76", "-compression_level", "6", "-update", "1",
                 os.path.join(ROOT, "public/finale-still.webp")])
            os.remove(png)
            print(f"  still: frame {i} -> {os.path.getsize(avif_out) / 1024:.0f} KB avif")
